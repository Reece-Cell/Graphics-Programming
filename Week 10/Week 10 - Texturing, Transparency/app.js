...
function updateAndRender() {
    ...
    for (var i = 0; i < sphereGeometryList.length; ++i) {
        ...
        var spherepos = sphere.getPosition();
    
        var distance = spherepos.subtract(cameraPosition).lengthSqr();
        
        distanecs.push({ geometry: sphere, distance: distance });
    }
    distanecs.sort(function(a, b) {
        //Accidentally sorted in the reverse of what I wanted but...
        return b.distance - a.distance;
    });
    
    // Render the sphere geometries in order of distance from the camera
    for (var i = 0; i < distanecs.length; ++i) {
        //By inverting the index it works anyway! Yay!
        distanecs[distanecs.length - i - 1].geometry.render(camera, projectionMatrix, textureShaderProgram);
    }
    // todo #6
