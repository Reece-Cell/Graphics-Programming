...
function createScene() {
    ...
    var translation1 = new Matrix4().makeTranslation(0,-2,-10);
    var rotation = new Matrix4().makeRotationX(90);
    var scale1 = new Matrix4().makeScale(10,10,10);
    groundGeometry.worldMatrix = translation1.multiply(scale1.multiply(rotation));
    //I know this code is fuck ugly but it finally worked so i'm happy
    // todo #2 - rotate and scale the quad to make it "ground-like"
    groundGeometry.worldMatrix.multiply(new Matrix4().makeScale(12, 12, 12).makeTranslation(0, 0, 0));
    ...
    var sphereTranslate = new Matrix4().makeTranslation(0,0,-5);
    sphereGeometry.worldMatrix = sphereTranslate.multiply(sphereScale);
...
function updateAndRender() {
    ...
    var range = Math.cos(time.secondsElapsedSinceStart);
    var colorValue = (range + 1) / 2;
    if(colorswap == true){
        increment += 0.01;
        if(increment > 1){
            colorswap = false;
    ...
    gl.uniform4f(colorProgram.uniforms.colorUniform, colorValue - increment, 1.0 - colorValue - increment, colorValue * range - increment, 1.0);
